...
"""
Created on Wed May  8 08:57:23 2024
...
# -*- coding: utf-8 -*-
"""
Created on Thu May  2 10:58:22 2024
@author: esterserrano
"""
import pandas as pd
...
csv_file_path = '/Users/Mathew/Documents/Current analysis/Spirosomes/Example/D1_Green.csv'
# Colour map:
...
df = df*0.0265/0.96
df.to_csv(csv_file_path[:-4]+'_photons.csv')
...
for i,col in enumerate(df.columns):
    ...
    ax.hist(df[col], bins=200, range=[1,2760.41], cumulative=True, density=True, alpha=0.7, label=col, fill=False,histtype='step',color=color)
# Set labels and legend
ax.set_xlabel('Total intensity of spirosome (photons)')
...
for i, col in enumerate(df.columns):
    ...
    axs[i].hist(df[col].dropna(), bins=50, range=[1,2760.41], alpha=0.7, color='skyblue', edgecolor='black')
    axs[i].set_title(col)
    axs[i].set_xlabel('Intensity of spirosome (photons)',fontname='Arial', fontsize=12)
...
for i, col in enumerate(df.columns):
    ...
    hist, bins = np.histogram(df[col].dropna(), bins=50, range=[1, 2760.41])
    # Normalize histogram counts
    '''hist_normalized = hist / hist.sum()'''
    # Store histogram data in the dictionary
    hist_data[col] = hist
    # Plot normalized histogram
    plt.bar(bins[:-1], hist, width=(bins[1] - bins[0]), alpha=0.7, color='skyblue', edgecolor='black')
    plt.title(col)
    plt.xlabel('Intensity of spirosome (photons)', fontname='Arial', fontsize=12)
